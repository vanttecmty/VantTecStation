...
def obstacleAvoidance(course=None, teamCode=None):
    ...
    if course == "courseA":
        if pattern.match(teamCode):
            ...
            return json.dumps({"error": True, "msg": "cannot find team"}), 404, {"ContentType":"application/json"}
    elif course == "courseB":
        if pattern.match(teamCode):
            print("hello")
        else:
        ...
        return json.dumps({"error": True, "msg": "Cannot find course"}), 404, {"ContentType":"application/json"}
